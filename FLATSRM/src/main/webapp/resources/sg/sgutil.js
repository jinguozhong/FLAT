...
var Jgzajax={
	request:function(options,params){
		var opts=$.extend({},{
			...
			complete:function(){tzUtil.animates($("#tzloading"),"slideUp");},
			success:function(){},
...
/**
 * 给form表单中的数据批量赋值，但此方式不支持checkbox的赋值
 * @param $form
 * @param jsonData
 */
function formAssignment($form,jsonData){ 
 $.each(jsonData,function(key,value){  
	 // console.log("name:"+key+"; value:"+value); 
 	var type=$form.find("[name="+key+"]").attr("type");
 	//获取是否是字典字段
 	var dictionary=$form.find("[name="+key+"]").attr("dictionary"); 
 	if(dictionary=='true'){//判断是否是字典字段
 		 var arr=new Array();
 		 arr=value.split(",");
 		$form.find("[name="+key+"]").data("id",arr[0])
 		$form.find("[name="+key+"]").val(arr[1]);
 	}else if(type=="radio"){ 
 	   $form.find("[name="+key+"][value="+value+"]").attr("checked",true);  
 	}else{
  ...
  * @param $form
  */
function formObtain($form){
	var datas={} 
	var dataArr=$form.serializeArray();   
	//console.log(JSON.stringify(dataArr));
	$.each(dataArr, function(i, item){
		var dictionary= $form.find("[name="+item.name+"]").attr("dictionary");
		if(dictionary=='true'){//判断是否是字典字段 
			datas["pId"]=$form.find("[name="+item.name+"]").data("id");
		}else{ 
...
//获取指定元素的宽度
function getSpecifiedWidth(obj){
	var $obj=$(obj);
	return $obj.innerWidth();
}
//获取指定元素的高度
function getSpecifiedHeight(obj){
	var $obj=$(obj);
