...
$.tzIframe = function(options){
	var opts = $.extend({},$.tzDialog.methods,$.tzDialog.defaults,options);
	var $dialog = opts.init(opts);
	$dialog.find(".tzdialog_message").html("<div  class='tm-dialog-loading tmWindow_loading'><img src=''><label style='font-size:12px;'>请稍后，正在加载中...</label></div>");
	...
	if(opts.showButton){
		...
		$dialog.find(".tzdialog_message").height(opts.height-89-h).css("padding","1px").append(iframe);
	}else{  
		$dialog.find(".tzdialog_message").height(opts.height-49-h).css("padding","1px").append(iframe);
	}
	...
	$dialog.find(".tzdialog_ok").off("click").on("click",function(){
		...
		if(opts.callback)opts.callback(true,iframe.contentWindow,$dialog,opts);
	});
	$dialog.find(".tzdialog_cancel,.delete").off("click").on("click",function(){
		
		if(opts.callback)opts.callback(false,iframe.contentWindow,$dialog,opts);
	});
};
...
$.tzDialog = function (options){
	var opts = $.extend({},$.tzDialog.methods,$.tzDialog.defaults,options);
	var $dialog=opts.init(opts);
	$dialog.find("#tzdialog_messageID").css({"padding": "2px 5px"});
	$dialog.find("#tzdialog_messageID").html(opts.content);
	$dialog.find(".tzdialog_ok").off("click").on("click",function(){
		
		if(opts.callback)opts.callback(true,$dialog,opts);
	});
	$dialog.find(".tzdialog_cancel,.delete").off("click").on("click",function(){
		
		if(opts.callback)opts.callback(false,$dialog,opts);
	});
...
$.tzDialog.methods = {
	...
	params:function($dialog,opts){
		...
		$dialog.find(".tzdialog_message").css({"textAlign":"center"}).height(opts.height-135);
		//弹出层居中
	...
	template : function(opts){
		...
		"			<div class='tzdialog_message' id='tzdialog_messageID'>" +
		...
		"		</div>"+
		"	</div>");
		$("body").append($dialog).append("<div class='tmui-overlay' style=''></div>");
