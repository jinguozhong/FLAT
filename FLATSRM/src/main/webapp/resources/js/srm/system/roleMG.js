 ...
 * jqgrid表格编辑模板
 */
var content="<div id='dialog-form-user'>"+
...
"		  			</tr>"+
"		  			<tr>"+
"		  				<th class='lableWidth'><lable for='name'>用户:</lable></th>"+
"		  				<th class='thWidth'><input type='text' name='name' id='name' class='form-control' placeholder='用户'"+
"		      				onclick='roleMG.onclickRole(this)'>"+
"		      			</th>"+
"		  			</tr>"+
"		  		</tbody>"+					
"		  	</table> "+
...
var roleMG={ 
		...
		ztreeIni:function($ztreeId){ 
	 		...
	 		var asyncSetting={
	 		        ...
	 		        callback:{  
	 		        	onClick: roleMG.zTreeOnClick,//点击事件
		...
		zTreeOnClick:function(event, treeId, treeNode){
			...
			if(id!=targetId){ 
				...
				Jgzajax.request({
					url:basePath+"/admin/system/roleMG/find",
					success:function(data){ 
						//console.log(data);
						if(data!=null){ 
							if(data.ex!=null){
								...
								//form表单赋值
								common.assignment(data.role,$("#information>form"),$("#system"));
								//表格赋值
		...
		zTreeOnClick1:function(event, treeId, treeNode){
			if(treeId=="roleGMTree1"){				
				$("#roleMG-content").find("#roleFrom").find("#pname").val( treeNode.name);
				$("#roleMG-content").find("#roleFrom").find("#pname").data("id",treeNode.id);
			};
			if(treeId=="roleGMUserTree"){
				var $obj=$("#dialogFrom"); 
	   ...
	   jqgridIni:function($obj){ 
			$obj.jqGrid({       
				...
				colModel:[ 
	                ...
	                { label: "用户", name: "name", width: 240,editable: true}
				],  
				viewrecords : true,  
				altRows: true,  
				rownumbers:true,
				multiselect: true, 
		        multiboxonly: true,
				pager:"#user-gridPager"
	        }); 
			/**
			 * jqgrid 编辑通用样式 start
			 */ 
			//navButtons
			$obj.jqGrid('navGrid',"#user-gridPager",{ 	//navbar options 
					editfunc: roleMG.jqgridEdit,
					editicon : 'ace-icon fa fa-pencil blue',  
					addfunc:roleMG.jqgridAdd,
					addicon : 'ace-icon fa fa-plus-circle purple',
					delfunc: roleMG.formDeleRow,
			 ...
			 */
			//隐藏表格中的工具栏目
			$("#roleMG-content").find("#user-gridPager_left").children().hide(); 
			...
			$("#pname").click(function(){
				...
				if(isEmpty(para)||para!="readonly"){ 
					//ztree异步加载
			 		...
			 		//显示下拉框
					common.pnameDowntree("roleMG-content","roleFrom","roleGMTree1","system/roleMG","pname",asyncSetting);
		...
		onclickRole:function(){
			//ztree异步加载
	 		var asyncSetting={
	 		        ...
	 		        callback:{  
	 		        	onClick: roleMG.zTreeOnClick1,//点击事件
	 		        	beforeExpand:common.beforeExpand,//异步加载父节点展开
	 		        	onAsyncError:common.onAsyncError//异步加载发生异常
	 		        }   
	 		};
	 		//显示下拉框
			common.pnameDowntree("bodyId","dialogFrom","roleGMUserTree","system/userMG","name",asyncSetting); 
		...
		validation:function($err){  
			//basicFrom的表单验证
			 $("#roleMG-content").find("#roleFrom").Validform({    
			 	tiptype:function(msg,o,cssctl){ //提示位置
			 	  cssctl($err,o.type);
			 	  $err.text(msg);  
				},
				callback:function(form){   
					roleMG.saveForm(form); //保存
					return false;
				} 
			});  
		},
		saveForm:function(form){ 
			...
			var $management=$("#roleMG-content");
			var $basicPills=$management.find("#content");
			
			 //获取参数 
			var fromData=formObtain($basicPills.find("#roleFrom")); 
			
			//获取表格中的数据
			var roleDatas=common.getFormRow('roleMG-content','user-gridTable'); 
			//获取父节点
			...
			if(nodes[0]!=null&&nodes[0].isParent && eval(isParent)==false&&nodes[0].children!=null){
				...
				var json={"basicFrom":fromData,"userDatas":roleDatas}; 
				//console.log(json);
				var datas=JSON.stringify(json);  
				if(isNotEmpty(datas)){
					Jgzajax.request({ 
						url:basePath+"/admin/system/roleMG/save",
						async: false,//设置为同步
						success:function(data){
							if(data.indexOf("SaveSuccess")>=0){ 
								var paramId=data.substring(12);  
								//调用ztree新增保存方法
								treeObj.addNodes(parentNode,{id:paramId,name:json.basicFrom.code+" "+json.basicFrom.name});  
								$basicPills.find("#id").val(paramId);
								//工具栏的显示隐藏的控制
								common.toolbarControl("save",$management,$basicPills); 
								$management.find("#user-gridPager_left").children().hide(); 
								//覆盖层取消
								$("#overlay").remove();	
								//覆盖层取消end
								...
								//调用ztree保存方法 
								nodes[0].name=json.basicFrom.code+" "+json.basicFrom.name;
